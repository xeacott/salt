...
def set_computer_name(name):
    ...
    if windll.kernel32.SetComputerNameExW(
        ...
        ret = {"Computer Name": {"Current": _get_computer_name()}}
        pending = _get_pending_computer_name()
...
def get_pending_reboot():
    ...
    checks = (
        _get_pending_update,
        _get_pending_windows_update,
        _get_pending_update_exe_volatile,
        _get_pending_file_rename,
        _get_pending_servermanager,
        _get_pending_component_servicing,
        _get_pending_dvd_reboot,
        _get_reboot_required_witnessed,
        _get_pending_computer_name,
        _get_pending_domain_join,
    )
...
def get_pending_reboot_details():
    ...
    """
    return {
        "Pending Component Servicing": _get_pending_component_servicing(),
        "Pending Computer Rename": _get_pending_computer_name() is not None,
        "Pending DVD Reboot": _get_pending_dvd_reboot(),
        "Pending File Rename": _get_pending_file_rename(),
        "Pending Join Domain": _get_pending_domain_join(),
        "Pending ServerManager": _get_pending_servermanager(),
        "Pending Update": _get_pending_update(),
        "Pending Windows Update": _get_pending_windows_update(),
        "Reboot Required Witnessed": _get_reboot_required_witnessed(),
        "Volatile Update Exe": _get_pending_update_exe_volatile(),
    }
...
def _get_pending_computer_name():
    """
...
def _get_pending_component_servicing():
    """
...
def _get_pending_servermanager():
    """
...
def _get_reboot_required_witnessed():
    """
...
def _set_reboot_required_witnessed():
    """
...
def _get_pending_update_exe_volatile():
    """
...
def _get_pending_windows_update():
    """
