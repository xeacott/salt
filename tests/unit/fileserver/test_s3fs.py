...
class S3fsConfigTestCase(TestCase, LoaderModuleMockMixin):
    def setup_loader_modules(self):
        ...
        return {s3fs: {"__opts__": opts, "__utils__": {"s3.query": MagicMock()}}}
    ...
    def test_get_file_from_s3_path_argument(self):
        ...
        with patch("os.path.isfile", MagicMock(return_value=False)):
            ...
            s3fs.__utils__["s3.query"].assert_called_with(
                bucket="bucket_name",
                https_enable=None,
                key=S3_KEY,
                keyid=S3_KEYID,
    ...
    def test_get_file_from_s3_with_spaces(self):
        ...
        with patch("os.path.isfile", MagicMock(return_value=False)):
            with patch.dict(s3fs.__utils__, {"s3.query": s3_utils.query}):
                with patch(
                    "salt.utils.aws.get_location",
                    MagicMock(return_value="dummy_location"),
                ):
